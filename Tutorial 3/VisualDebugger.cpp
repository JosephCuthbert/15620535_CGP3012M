...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(0.0f, 17.0f, 20.0f), PxVec3(0.f,-.45f,-1.f), 5.f);
	...
	void HUDInit()
	{
		...
		hud.AddLine(HUDState::HELP, "Score: ");
		...
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Padels");
		hud.AddLine(HELP, " V - Left, B - Right");
		//add a pause screen
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "");
	...
	void RenderScene()
	{
		...
		hud.Clear();
		hud.AddLine(UPDATE, "");
		hud.AddLine(UPDATE, " Score: " + std::to_string(scene->score));
		hud.AddLine(UPDATE, "");
		hud.AddLine(UPDATE, " Tries: " + std::to_string(scene->deaths));
		hud.AddLine(UPDATE, "");
		hud.AddLine(UPDATE, " Padels");
		hud.AddLine(UPDATE, " V - Left, B - Right");
		hud.AddLine(UPDATE, "");
		hud.AddLine(UPDATE, "Space to Launch");
		hud.AddLine(UPDATE, "");
		hud.AddLine(UPDATE, "F9 - Reset");
		hud.AddLine(UPDATE, "position");
		
		hud.FontSize(0.018f);
		hud.Color(PxVec3(0.f /255.f,0.f/255.f, 0.f /255.f));
	...
	void UserKeyPress(int key)
	{
		...
		case 'R':
			...
			break;
		case 'B':
			scene-> PadelForce(3); // Add force to left flipper
			cerr << "B pressed" << endl;
			break;
		case 'V':
			scene-> PadelForce(2); //Adds force to left padel
			cerr << "V pressed" << endl;
			break;
		case ' ':
			scene->AddForce();
	...
	void UserKeyRelease(int key)
	{
		...
		case 'B':
			scene->PadelForce(4); //Adds force to left padel
			break;
		case 'V':
			scene->PadelForce(1); //Adds force to left padel
			break;
		default:
			break;
		}
